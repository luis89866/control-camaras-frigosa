...
def cargar_datos_env(sheet_name):
    """Carga los datos con caché temporal de 10 segundos para blindar la app contra el Error 429"""
    try:
...
def obtener_lote_por_fecha(fecha_dt):
    """Calcula el lote del año 2026 de forma matemática instantánea sin consultar Google Sheets"""
    try:
        dia_del_anio = fecha_dt.timetuple().tm_yday
        anio_dos_digitos = fecha_dt.strftime("%y")
        return f"LT{anio_dos_digitos}.{dia_del_anio:03d}"
    except:
        return f"LT-{fecha_dt.strftime('%Y%m%d')}"
...
def render_module(user, get_sheet, cargar_datos):
    ...
    # Lista de calibres fija en código para máxima velocidad y sin errores de API
    lista_calibres = [
        "-",
        "0-50 GR/PZA",
        "50-100 GR/PZA",
    ...
    with st.expander("📥 1. Ingresos de Datos (Plaqueros, Túneles e IQF)", expanded=True):
        st.caption("Registre la carga del equipo. El lote se calcula automáticamente al instante.")
        
        col_d1, col_d2 = st.columns(2)
        with col_d1:
            tipo_equipo = st.selectbox("Seleccione Tipo de Equipo:", ["Plaquero (P1 - P18)", "Túnel (1, 2 y 3)", "Equipo IQF (1 y 2)"], key="tipo_eq_sel_v66")
            if tipo_equipo == "Plaquero (P1 - P18)":
                plaquero_sel = st.selectbox("Nº de Plaquero:", [f"P{i}" for i in range(1, 19)], key="sel_plaquero_reg_v66")
            elif tipo_equipo == "Túnel (1, 2 y 3)":
                plaquero_sel = st.selectbox("Nº de Túnel:", ["TÚNEL 1", "TÚNEL 2", "TÚNEL 3"], key="sel_tunel_reg_v66")
            else:
                plaquero_sel = st.selectbox("Nº de IQF:", ["IQF 1", "IQF 2"], key="sel_iqf_reg_v66")
                
            fecha_ingreso_obj = st.date_input("Fecha de Producción:", obtener_hora_peru(), key="f_prod_reg_v66")
            ...
            hora_inicio_str = st.text_input("Hora de Inicio (Ej: 08:00):", obtener_hora_peru().strftime("%H:%M"), key="h_inicio_prod_v66")
            
        with col_d2:
            st.markdown("##### Tiempo de Congelación (Ej: 2:45 o 0:02 min):")
            tiempo_input_str = st.text_input("Tiempo de Congelación:", "2:45", key="t_cong_libre_v66")
        ...
        with col_p1:
            presentacion_sel = st.selectbox("Presentación / Producto:", lista_presentaciones, key="sel_presentacion_v66")
        with col_p2:
            calibre_sel = st.selectbox("Calibre:", lista_calibres, key="sel_calibre_v66")
        with col_p3:
            bandejas_cant = st.number_input("Cantidad de Bandejas:", min_value=1, step=1, value=50, key="num_bandejas_v66")

        if st.button("🚀 Registrar / Agregar Presentación", use_container_width=True, key="btn_enviar_produccion_v66"):
            try:
                ws_env = get_env_sheet("ingreso_plaqueros")
    ...
    with st.expander("⚡ 2. Equipos Encendidos (Plaqueros, Túneles e IQF)", expanded=False):
        ...
        with col_c2:
            if st.button("🔄 Actualizar Cronograma", key="btn_actualizar_cronograma_v51", use_container_width=True):
                st.cache_data.clear()
                st.rerun()
        ...
        try:
            ...
            for idx, row in df_mostrar_tabla.iterrows():
                ...
                with col_t6:
                    if len(row['ids']) > 0:
                        if st.button("🔓 Liberar", key=f"lib_fix_sel_v34_{row['EQUIPO']}_{idx}"):
                            try:
                                ws_env = get_env_sheet("ingreso_plaqueros")
                                all_vals = ws_env.get_all_values()
    ...
    with st.expander("✏️ 3. Modificación o Eliminación de Registros", expanded=False):
        ...
        try:
            ...
            if not df_all_mod.empty and "fecha" in df_all_mod.columns:
                filtro_fecha_mod_obj = st.date_input("📅 Seleccionar Día a Gestionar:", obtener_hora_peru(), key="filtro_fecha_mod_input_v24")
                ...
                if not df_mod_filtrado.empty:
                    ...
                    id_a_editar = st.selectbox("Seleccione el ID del registro a editar/eliminar:", df_mod_filtrado["id_produccion"].tolist(), key="sel_id_mod_v44")
                    ...
                    with col_btn1:
                        if st.button("💾 Guardar Cambios (Modificar)", type="primary", key="btn_guardar_mod_v44", use_container_width=True):
                            ws_env = get_env_sheet("ingreso_plaqueros")
                            all_vals = ws_env.get_all_values()
                            fila_tabla = -1
                            for f_idx, f_vals in enumerate(all_vals):
                    ...
                    with col_btn2:
                        if st.button("🗑️ Eliminar Registro", type="secondary", key="btn_eliminar_reg_v44", use_container_width=True):
                            ws_env = get_env_sheet("ingreso_plaqueros")
                            all_vals = ws_env.get_all_values()
                            fila_tabla = -1
                            for f_idx, f_vals in enumerate(all_vals):
    ...
    with st.expander("📊 4. Resumen de Producción y Materia Prima (MP)", expanded=False):
        ...
        with col_mp1:
            fecha_mp_obj = st.date_input("Fecha MP:", obtener_hora_peru(), key="fecha_mp_input_v19")
        ...
        with col_f1:
            filtro_fecha_obj = st.date_input("Filtrar por Fecha:", obtener_hora_peru(), key="filtro_fecha_hist_v60")
        with col_f2:
            try:
            ...
            filtro_lote_sel = st.selectbox("Filtrar por Lote:", lotes_disponibles, key="filtro_lote_sel_v60")
    ...
    with st.expander("🕒 5. Histograma de Plaqueros del Día", expanded=False):
        ...
        with col_h1:
            fecha_histo_obj = st.date_input("📅 Seleccionar Día de Histograma:", obtener_hora_peru(), key="fecha_histo_input_v16")
    ...
    with st.expander("📊 6. Rendimiento por Presentación y Calibre", expanded=False):
        ...
        with col_r6_1:
            fecha_r6_obj = st.date_input("📅 Seleccionar Fecha:", fecha_def_r6, key="fecha_r6_input_v16")
        ...
        try:
            ...
            if not df_env_r6.empty and "fecha" in df_env_r6.columns:
                ...
                if not df_r6_dia.empty:
                    ...
                    st.success(f"📌 **Suma Total del Día:** `{suma_kilos_totales:,.2f} kg` (Rendimiento Acumulado: `{suma_rend_totales:.2f}%`)")
                else:
    ...
    with st.expander("📊 7. Resumen por Subfamilia (Presentación)", expanded=False):
        ...
        with col_r7_1:
            fecha_r7_obj = st.date_input("📅 Seleccionar Fecha:", fecha_def_r7, key="fecha_r7_input_v13")
    ...
    with st.expander("⏱️ 8. Control y Registro de Tiempos Muertos / Paradas", expanded=False):
        ...
        with col_par1:
            fecha_parada_obj = st.date_input("Fecha de Parada:", obtener_hora_peru(), key="fecha_parada_input_v10")
            fecha_parada_str = fecha_parada_obj.strftime("%Y-%m-%d")
            
            tipo_eq_par = st.selectbox("Equipo Afectado:", ["Plaquero (P1 - P18)", "Túnel (1, 2 y 3)", "Equipo IQF (1 y 2)"], key="tipo_eq_par_v10")
            if tipo_eq_par == "Plaquero (P1 - P18)":
                eq_par_sel = st.selectbox("Nº de Plaquero:", [f"P{i}" for i in range(1, 19)], key="eq_par_plaquero_v10")
            elif tipo_eq_par == "Túnel (1, 2 y 3)":
                eq_par_sel = st.selectbox("Nº de Túnel:", ["TÚNEL 1", "TÚNEL 2", "TÚNEL 3"], key="eq_par_tunel_v10")
            else:
                eq_par_sel = st.selectbox("Nº de IQF:", ["IQF 1", "IQF 2"], key="eq_par_iqf_v10")
        ...
        with col_par2:
            motivo_parada = st.selectbox("Motivo de Parada / Demora:", [
                ...
                "Otro"
            ], key="motivo_parada_sel_v10")
            
            minutos_muertos = st.number_input("Tiempo Muerto (en Minutos):", min_value=1, step=10, value=30, key="min_muertos_input_v10")
            observaciones_parada = st.text_input("Observaciones / Comentarios:", "", key="obs_parada_input_v10")

        if st.button("💾 Registrar Parada / Tiempo Muerto", type="primary", use_container_width=True, key="btn_guardar_parada_v10"):
            if motivo_parada == "Otro" and not observaciones_parada.strip():
                ...
                try:
                    ...
                    obs_final = observaciones_parada.strip() if observaciones_parada.strip() else "-"
                    
                    ws_paradas.append_row([
                        id_par, fecha_parada_str, eq_par_sel, motivo_parada, str(minutos_muertos), obs_final
                    ])
                    st.cache_data.clear()
        ...
        col_fpar1, col_fpar2 = st.columns(2)
        with col_fpar1:
            filtro_par_obj = st.date_input("📅 Filtrar Historial por Fecha:", obtener_hora_peru(), key="filtro_fecha_paradas_v6")
        filtro_par_str = filtro_par_obj.strftime("%Y-%m-%d")

        try:
            df_paradas = cargar_datos_env("PARADAS")
            if not df_paradas.empty and "fecha" in df_paradas.columns:
                df_paradas_filtrado = df_paradas[df_paradas["fecha"].astype(str).str.strip() == filtro_par_str]
                if not df_paradas_filtrado.empty:
                    st.dataframe(df_paradas_filtrado, use_container_width=True)
                else:
                    st.info(f"ℹ️ No hay registros de paradas para la fecha {filtro_par_str}.")
            else:
