...
def render_module(user, get_sheet, cargar_datos):
    ...
    # Lista de calibres fija en código para máxima velocidad y cero errores de API
    lista_calibres = [
        "-",
        "0-50 GR/PZA",
        "50-100 GR/PZA",
    ...
    with st.expander("📥 1. Ingresos de Datos (Plaqueros, Túneles e IQF)", expanded=True):
        ...
        with col_d1:
            tipo_equipo = st.selectbox("Seleccione Tipo de Equipo:", ["Plaquero (P1 - P18)", "Túnel (1, 2 y 3)", "Equipo IQF (1 y 2)"], key="tipo_eq_sel_v65")
            if tipo_equipo == "Plaquero (P1 - P18)":
                plaquero_sel = st.selectbox("Nº de Plaquero:", [f"P{i}" for i in range(1, 19)], key="sel_plaquero_reg_v65")
            elif tipo_equipo == "Túnel (1, 2 y 3)":
                plaquero_sel = st.selectbox("Nº de Túnel:", ["TÚNEL 1", "TÚNEL 2", "TÚNEL 3"], key="sel_tunel_reg_v65")
            else:
                plaquero_sel = st.selectbox("Nº de IQF:", ["IQF 1", "IQF 2"], key="sel_iqf_reg_v65")
                
            fecha_ingreso_obj = st.date_input("Fecha de Producción:", obtener_hora_peru(), key="f_prod_reg_v65")
            ...
            hora_inicio_str = st.text_input("Hora de Inicio (Ej: 08:00):", obtener_hora_peru().strftime("%H:%M"), key="h_inicio_prod_v65")
            
        with col_d2:
            st.markdown("##### Tiempo de Congelación (Ej: 2:45 o 0:02 min):")
            tiempo_input_str = st.text_input("Tiempo de Congelación:", "2:45", key="t_cong_libre_v65")
        ...
        with col_p1:
            presentacion_sel = st.selectbox("Presentación / Producto:", lista_presentaciones, key="sel_presentacion_v65")
        with col_p2:
            calibre_sel = st.selectbox("Calibre:", lista_calibres, key="sel_calibre_v65")
        with col_p3:
            bandejas_cant = st.number_input("Cantidad de Bandejas:", min_value=1, step=1, value=50, key="num_bandejas_v65")

        if st.button("🚀 Registrar / Agregar Presentación", use_container_width=True, key="btn_enviar_produccion_v65"):
            try:
                ws_env = get_env_sheet("ingreso_plaqueros")
    ...
    with st.expander("⚡ 2. Equipos Encendidos (Plaqueros, Túneles e IQF)", expanded=False):
        ...
        with col_c2:
            if st.button("🔄 Actualizar Cronograma", key="btn_actualizar_cronograma_v50", use_container_width=True):
                st.rerun()
        ...
        try:
            ...
            for idx, row in df_mostrar_tabla.iterrows():
                ...
                with col_t6:
                    if len(row['ids']) > 0:
                        if st.button("🔓 Liberar", key=f"lib_fix_sel_v33_{row['EQUIPO']}_{idx}"):
                            try:
                                ws_env = get_env_sheet("ingreso_plaqueros")
                                all_vals = ws_env.get_all_values()
    ...
    with st.expander("✏️ 3. Modificación o Eliminación de Registros", expanded=False):
        ...
        try:
            ...
            if not df_all_mod.empty and "fecha" in df_all_mod.columns:
                filtro_fecha_mod_obj = st.date_input("📅 Seleccionar Día a Gestionar:", obtener_hora_peru(), key="filtro_fecha_mod_input_v23")
                ...
                if not df_mod_filtrado.empty:
                    ...
                    id_a_editar = st.selectbox("Seleccione el ID del registro a editar/eliminar:", df_mod_filtrado["id_produccion"].tolist(), key="sel_id_mod_v43")
                    ...
                    with col_btn1:
                        if st.button("💾 Guardar Cambios (Modificar)", type="primary", key="btn_guardar_mod_v43", use_container_width=True):
                            ws_env = get_env_sheet("ingreso_plaqueros")
                            all_vals = ws_env.get_all_values()
                            fila_tabla = -1
                            for f_idx, f_vals in enumerate(all_vals):
                    ...
                    with col_btn2:
                        if st.button("🗑️ Eliminar Registro", type="secondary", key="btn_eliminar_reg_v43", use_container_width=True):
                            ws_env = get_env_sheet("ingreso_plaqueros")
                            all_vals = ws_env.get_all_values()
                            fila_tabla = -1
                            for f_idx, f_vals in enumerate(all_vals):
    ...
    with st.expander("📊 4. Resumen de Producción y Materia Prima (MP)", expanded=False):
        ...
        with col_mp1:
            fecha_mp_obj = st.date_input("Fecha MP:", obtener_hora_peru(), key="fecha_mp_input_v18")
        ...
        with col_f1:
            filtro_fecha_obj = st.date_input("Filtrar por Fecha:", obtener_hora_peru(), key="filtro_fecha_hist_v59")
        with col_f2:
            try:
            ...
            filtro_lote_sel = st.selectbox("Filtrar por Lote:", lotes_disponibles, key="filtro_lote_sel_v59")
    ...
    with st.expander("🕒 5. Histograma de Plaqueros del Día", expanded=False):
        ...
        with col_h1:
            fecha_histo_obj = st.date_input("📅 Seleccionar Día de Histograma:", obtener_hora_peru(), key="fecha_histo_input_v15")
    ...
    with st.expander("📊 6. Rendimiento por Presentación y Calibre", expanded=False):
        ...
        with col_r6_1:
            fecha_r6_obj = st.date_input("📅 Seleccionar Fecha:", fecha_def_r6, key="fecha_r6_input_v15")
    ...
    with st.expander("📊 7. Resumen por Subfamilia (Presentación)", expanded=False):
        ...
        with col_r7_1:
            fecha_r7_obj = st.date_input("📅 Seleccionar Fecha:", fecha_def_r7, key="fecha_r7_input_v12")
    ...
    with st.expander("⏱️ 8. Control y Registro de Tiempos Muertos / Paradas", expanded=False):
        ...
        with col_par1:
            fecha_parada_obj = st.date_input("Fecha de Parada:", obtener_hora_peru(), key="fecha_parada_input_v9")
            fecha_parada_str = fecha_parada_obj.strftime("%Y-%m-%d")
            
            tipo_eq_par = st.selectbox("Equipo Afectado:", ["Plaquero (P1 - P18)", "Túnel (1, 2 y 3)", "Equipo IQF (1 y 2)"], key="tipo_eq_par_v9")
            if tipo_eq_par == "Plaquero (P1 - P18)":
                eq_par_sel = st.selectbox("Nº de Plaquero:", [f"P{i}" for i in range(1, 19)], key="eq_par_plaquero_v9")
            elif tipo_eq_par == "Túnel (1, 2 y 3)":
                eq_par_sel = st.selectbox("Nº de Túnel:", ["TÚNEL 1", "TÚNEL 2", "TÚNEL 3"], key="eq_par_tunel_v9")
            else:
                eq_par_sel = st.selectbox("Nº de IQF:", ["IQF 1", "IQF 2"], key="eq_par_iqf_v9")
        ...
        with col_par2:
            motivo_parada = st.selectbox("Motivo de Parada / Demora:", [
                ...
                "Otro"
            ], key="motivo_parada_sel_v9")
            
            minutos_muertos = st.number_input("Tiempo Muerto (en Minutos):", min_value=1, step=10, value=30, key="min_muertos_input_v9")
            observaciones_parada = st.text_input("Observaciones / Comentarios:", "", key="obs_parada_input_v9")

        if st.button("💾 Registrar Parada / Tiempo Muerto", type="primary", use_container_width=True, key="btn_guardar_parada_v9"):
        ...
        with col_fpar1:
            filtro_par_obj = st.date_input("📅 Filtrar Historial por Fecha:", obtener_hora_peru(), key="filtro_fecha_paradas_v5")
